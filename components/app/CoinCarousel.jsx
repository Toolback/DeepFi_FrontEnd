...
const CoinCarousel = ({ coins, displayNbMobile, displayNbDesktop, mode }) => {
  ...
  function formatBalance(coin, mode) {
    ...
    if (mode === 3)
      ...
      dec = ethers.utils.formatUnits(bal, coin.dec);
    let resBal;
    if (dec.length > 12) {
      popUpActive = true;
      resBal = dec.slice(0, 12) + '...';
    }
    else {
      popUpActive = false;
      resBal = dec
    }
    return { resBal, dec, popUpActive };
  }

  const SingleCoinView = ({ coin, mode }) => {
    let { resBal, dec, popUpActive } = formatBalance(coin, mode);
    return (<>
      <div className="  flex flex-col items-center px-4 ">
        <img className="z-30 h-[44px]" src={coin.url} />
        <dt className="truncate text-sm font-medium text-gray-300">{coin.name}</dt>
        {popUpActive ?

          <div className="group flex relative">
            <span className=" mt-1 text-sm tracking-tight">{resBal}</span>
...
opacity-0">
          ...
          :
          <dd className="mt-1 text-sm  tracking-tight">{resBal}</dd>
        }
      </div>
    </>)
  }
  ...
  return (
    <div className="py-4">
      {dataLoaded === false ? (
        ...
        <>
          {displayedCoins.length === 1 ? <SingleCoinView coin={displayedCoins[0]} mode={mode} /> : <>
